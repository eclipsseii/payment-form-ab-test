...
Файл `dannie-for-abtest.xlsx`, период теста - апрель 2024, два листа:
...
Одна строка в Платежах это одна попытка оплаты. У пользователя может быть несколько попыток. Пустое время шага означает, что пользователь до него не дошёл.
